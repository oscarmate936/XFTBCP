...
class MotorMatematico:
    def __init__(self, league_avg=2.5, draw_rate_real=0.25, liga_id=None, round_name=None):
        ...
        # Optimizar rho con un método más robusto
        self.rho = self.optimizar_rho_mejorado(draw_rate_real, self.round_avg)
        self.pi_l = 0.06
        self.pi_v = 0.09
    ...
    def optimizar_rho_mejorado(self, empates_reales_pct, prom_liga):
        """
        Optimiza rho minimizando la diferencia entre la probabilidad de empate teórica
        y la observada, usando una búsqueda con restricciones.
        """
        l_prom = max(0.1, prom_liga / 2)
        def prob_empate_teorico(rho_val):
            p_empate = 0
            for i in range(8):  # hasta 7 goles
                p_i = poisson.pmf(i, l_prom)
                p_j = poisson.pmf(i, l_prom)
                if i == 0:
                    p_j *= max(0, 1 - (l_prom**2) * rho_val)
                elif i == 1:
                    p_j *= max(0, 1 - rho_val)
                p_empate += p_i * p_j
            return p_empate
        ...
        def loss(rho_test):
            return (prob_empate_teorico(rho_test[0]) - empates_reales_pct)**2

        res = minimize(loss, [0.0], bounds=[(-0.3, 0.2)], method='L-BFGS-B')
